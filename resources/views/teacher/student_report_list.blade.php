@php
    ...
    // {{ $date =strval(substr($item->report_from_date,0,8)),Carbon::parse($date)->thaidate('D j M y') }}
@endphp
                        ...
                        <tbody>
                            @foreach ($company as $item)
                                <tr class="align-middle">
                                    ...
                                    <td class="text-nowrap">{{ Carbon::parse($item->report_from_date)->thaidate('D j M y') }} เวลา {{substr($item->report_from_date,8,20)}} นาที</td>
                                    <td class="text-nowrap">{{ Carbon::parse($item->report_to_date)->thaidate('D j M y') }} เวลา {{substr($item->report_to_date,8,20)}} นาที</td>
                                    <td class="text-center" style="color:red;"><p class="text-red">ยังไม่เปิดให้ใช้</p>
                                ...
                                <form action="{{route('student.report.store')}}" method="POST" class="row">
                                  @csrf
                                    ...
                                    <div class="col-9 col-sm-7 mb-3">
                                        <input class="form-control datetimepicker" name="report_from_date" id="datetimepicker" type="text"
                                    ...
                                    <div class="col-9 col-sm-7 mb-3">
                                        <input class="form-control datetimepicker" name="report_to_date" id="datetimepicker" type="text"
                                            placeholder="วัน/เดือน/ปี เวลา"
                                            data-options='{"enableTime":true,"dateFormat":"d-m-y H:i","disableMobile":true}' />
                                    </div>
                                    ...
                                    <div class="col-9 col-sm-7 offset-3 mb-3">
                                        <div class="form-check mb-0 lh-1">
                                            <input class="form-check-input" type="checkbox" name="i_amOk" value="1" id="experience-current"
                                                checked="checked">
